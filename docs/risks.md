...
| RISK-002 | Primary thermistor failure causes unsafe control | High | Medium | Thermistor invalid/out-of-range is hard fault; startup plausibility check | Open |
| RISK-003 | Heater relay stuck ON | High | Low/Medium | After 2 min post-heater-off grace period, detect primary thermistor rise of 3 C over 5 min while heater command OFF; hard fault; independent cutoff recommended | Open |
...
| RISK-012 | Inaccurate primary thermistor due to VCC reference/voltage divider | Medium | Medium | Calibration constants, filtering, debug visibility; consider better reference or dedicated thermistor interface | Open |
