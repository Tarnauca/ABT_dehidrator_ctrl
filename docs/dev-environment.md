...
This document preserves the working WSL/devcontainer/PlatformIO/serial/Git setup notes.

## PlatformIO

- Prerequisites: PlatformIO Core or VS Code with PlatformIO extension.
- Current default environment: `megaatmega2560`.
- Current `platformio.ini` still contains legacy Uno/Nano environments from the starter project. They should be audited before removal, and `platformio.ini` must not be edited without explicit user confirmation.
...
## Dev Container In VS Code And WSL
...
Before modifying `.devcontainer`, `platformio.ini`, upload settings, serial settings, dependency configuration, or CI scripts:
