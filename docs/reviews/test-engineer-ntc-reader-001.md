# Test Engineer Review 001: NTC Reader And Calibration
...
The NTC conversion and interface-based reader structure was good, but the
...
- Changed the model to keep calibrated Celsius in a wide integer type during
  plausibility checks.
- Assign `NtcReading::tempC` only after the wide value is confirmed plausible
...
Resolution:
- Added a high-temperature direction test for `NtcHighFixedLow`.
...
- Split the backlog into completed interface-based model/reader work and a
  pending concrete Arduino NTC adapter after wiring is confirmed.
